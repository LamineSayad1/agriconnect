...
async function run() {
    ...
    const statuses = ['pending', 'Pending', 'confirmed', 'Confirmed', 'delivered', 'Delivered'];
    for (const s of statuses) {
        const { error } = await supabase.from('orders').insert({ buyer_id: buyerId, status: s, total_amount: 1 });
        results.push({ status: s, success: !error, error: error ? error.message : null });
    }
